...
public class ParallelNotificationMediatorFactory<TNotification> : INotificationMediatorFactory<TNotification> where TNotification : class
{
    ...
    public ParallelNotificationMediatorFactory(IEnumerable<INotificationHandler<TNotification>> handlers)
    {
        _handlers = handlers;
    }
    ...
    public INotificationMediator<TNotification> CreateMqMediator()
    {
        ...
        {
            var publishSize = 0;
            var executionPublishSequence = new SortedList<ServicingOrder, List<INotificationHandler<TNotification>>>(_handlers.Count());
